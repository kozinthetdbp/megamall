...
class JoomlArt_JmProductsSlider_Block_List extends Mage_Catalog_Block_Product_Abstract 
{
	...
	function getListTopRatedProducts($orderfeild='rating_summary', $order='desc', $perPage=NULL, $currentPage=1){
		...
		$products->getSelect()->joinLeft(
			array('_reviewed_order_table' => Mage::getConfig()->getTablePrefix().'review_entity_summary' ),
	...
	function getListMostViewedProducts(){			
        ...
        if($productFlatData == "1")
        {
            $products_collection->getSelect()->joinLeft(
                array('flat' => Mage::getConfig()->getTablePrefix().'catalog_product_flat_'.$storeId),
                "(e.entity_id = flat.entity_id ) ",
                array(
                    'flat.name AS name','flat.small_image AS small_image', 'flat.thumbnail AS thumbnail', 'flat.price AS price','flat.special_price as special_price','flat.special_from_date AS special_from_date','flat.special_to_date AS special_to_date'
	...
	function getListBestBuyProducts($fieldorder='ordered_qty', $order='desc'){		
		...
		Mage::getSingleton('catalog/product_status')->addVisibleFilterToCollection($products_collection);
    ...
    */
    ...
    public function addCategoryIdsFilter(&$collection, $matchType = 'OR'){
		$categoryIds = explode(",", $this->_config['catsid']);
		if(count($categoryIds) == 1 || $matchType == 'AND')
		{
